##shallow sequence comparison of spleen data
...
fnames = c("TAS-Seq.shallow-1","TAS-Seq.shallow-2","TAS-Seq.shallow-3", "BD.shallow-1", "BD.shallow-2", "BD.shallow-3")
...
percent.riboRNA = Matrix::colSums(mBC@assays$RNA@counts[riboRNA.genes, ])/Matrix::colSums(mBC@assays$RNA@counts)
...
#violin plot of ribosomal RNA proportion
p = VlnPlot(mBC, features="percent.riboRNA", pt.size=0)

p_legend = get_legend(p)

p = VlnPlot(mBC, features="percent.riboRNA", pt.size=0) +
  geom_boxplot(outlier.shape=NA, width=0.15) +
  theme_linedraw() + ylab("percent.riboRNA") +
  ggtitle("")+xlab("")+
  theme(plot.title=element_text(hjust = 0.5),
        legend.position = 'none', text=element_text(size=12), axis.text.x = element_text(angle=45, hjust=1))

ggsave(plot_grid(p, p_legend, rel_widths = c(0.5,0.4)), filename="riboRNA_spleen_VlnPlot.png", units="in", dpi=300,
       width=4, height=3.5, limitsize=FALSE)

#wilcox test of percent mito
hoge = mBC@meta.data
...
#wilcox test of percent riboRNA
hoge = mBC@meta.data
hoge$orig.ident = as.character(hoge$orig.ident)
hoge$percent.mito = round(hoge$percent.riboRNA, digits=5)
res = hoge %>% wilcox_test(percent.mito ~ orig.ident, exact = TRUE) %>% add_significance() %>% add_xy_position(x = "orig.ident")
colnames(res)[6]="W_statistic"
res=as.data.frame(res)
res=res[,c(2:8), drop=F]
write.table(res, file="stats_percent.riboRNA_spleen.txt", quote=F,  sep="\t", row.names=F, col.names=T)
