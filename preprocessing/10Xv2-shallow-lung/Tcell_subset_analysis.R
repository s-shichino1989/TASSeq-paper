...
hoge = mBC1@assays$RNA@counts[,rownames(mBC1@meta.data[mBC1@meta.data$seurat_clusters %in% c(4,20),])]
...
dims=1:10  ##manually set dims = 1:10 for better separation
...
mBC = FindClusters(object = mBC, resolution = 0.6, random.seed=42, verbose = FALSE)
DimPlot(mBC, reduction = "umap", pt.size=1, label = TRUE)
FeaturePlot(mBC, features=c("Cd8a", "Cd4", "Foxp3","Tcf7"), reduction="umap")
...
hoge = mBC1@assays$RNA@counts[,rownames(mBC1@meta.data[mBC1@meta.data$seurat_clusters %in% c(8),])]
mBC = CreateSeuratObject(hoge)
mBC = NormalizeData(mBC, scale.factor=1000000)
mBC = FindVariableFeatures(mBC, selection.method = "mvp", 
                           mean.cutoff=c(0.1, Inf), dispersion.cutoff=c(0.5, Inf))
...
DimPlot(mBC, reduction = "umap", pt.size=1, label = TRUE)
FeaturePlot(mBC, features=c("Trdc","Cd3e", "Cd4", "Cd8a", "Nkg7", "Il1rl1", "Rora","Csf2", "Mki67"), reduction="umap")

write.table(table(mBC@active.ident, mBC@meta.data$orig.ident), file="gdTcell_ILC2_Cellnumber.txt", sep="\t", quote=F, row.names=T, col.names=T)
save(mBC, file="gdTcell_ILC2_gate.rda")
