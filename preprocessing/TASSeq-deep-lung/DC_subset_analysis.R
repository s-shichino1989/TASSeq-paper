...
mBC = qread("./Seurat/UTlung_deep_Seurat.qs")
mBC1 = mBC

#DC

hoge = mBC1@assays$RNA@counts[,rownames(mBC1@meta.data[mBC1@meta.data$RNA_snn_res.3 %in% c(34),])]
...
mBC = FindClusters(object = mBC, resolution = 1.7, random.seed=42, verbose = FALSE)
DimPlot(mBC, reduction = "umap", pt.size=1, label = TRUE)
FeaturePlot(mBC, features=c("C1qa", "Cd209a", "Siglech", "Fcgr1", "Cx3cr1", "Ccr7"), reduction="umap")

write.table(table(mBC@active.ident, mBC@meta.data$orig.ident), file="cDC2_IM_gate1_Cellnumber.txt", sep="\t", quote=F, row.names=T, col.names=T)
save(mBC, file="cDC2_IM_gate.rda")
