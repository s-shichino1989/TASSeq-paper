---
title: "Seurat analysis report of mouse lung Smart-seq2 data"
date: "`r Sys.Date()`"
...
if (packageVersion("Seurat") == "2.3.4"){
  tablelist = lapply(tablelist, CreateSeuratObject, min.cells = 5, min.genes = 200)
...
if(!DBEC_output){
  if (packageVersion("Seurat") == "2.3.4"){
mBC = CreateSeuratObject(tablelist[[1]], min.cells=5, min.genes=200)
} else if (packageVersion("Seurat") >= "3.0"){
  mBC = CreateSeuratObject(tablelist[[1]], min.cells=5, min.features=200)
}
}
}
...
message("Performing DBEC-correction... This may take some time.")
 ...
 seed = 42
  ...
  for (i in 1:nsamples){
    ...
    fuga = CreateSeuratObject(x[[i]], min.cells=min.event+1, min.genes=1)
    fuga = fuga@assays$RNA@counts
    
    tmp1 = qlcMatrix::rowMax(fuga)
    tmp1 = as.vector(tmp1)
    
    tmp2 = tmp1 >= 2^minimum.max.expr
    tmp2 = fuga[tmp2,]
